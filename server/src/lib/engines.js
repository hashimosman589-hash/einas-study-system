...
// التوازي عبر الأجزاء قابل للضبط في .env (AI_PIPELINE_CONCURRENCY) — أقصى 6.
...
// قِيسَ محليًا: 6 لا يسبّب 429 (صفر إعادة محاولة) ويعالج الملفات الكبيرة (>4 أجزاء) في موجات أقل.
// يُرشَّد لـ 1 فقط عند تقييد إضافي من المزود، ويُرفع حتى 6 عند ترقية الحساب.
const PIPELINE_CONCURRENCY = Math.max(1, Math.min(6, parseInt(process.env.AI_PIPELINE_CONCURRENCY || '2', 10) || 1));

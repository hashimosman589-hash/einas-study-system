...
router.get('/dashboard', (req, res) => {
  ...
  const lectureCount = db.prepare('SELECT COUNT(*) c FROM lectures WHERE user_id = ?').get(userId).c;
  const readyCount = db.prepare('SELECT COUNT(*) c FROM lectures WHERE user_id = ? AND status = ?').get(userId, 'ready').c;
  const examCount = db.prepare('SELECT COUNT(*) c FROM exams WHERE user_id = ?').get(userId).c;
  const questionCount = db.prepare('SELECT COUNT(*) c FROM questions WHERE user_id = ?').get(userId).c;
  ...
  // نسبة الإنجاز: محاضرات جاهزة محللة من إجمالي المرفوع + اختبارات منجزة إلي متوسط
  const completion = lecturesTarget(userId);
...
function lecturesTarget(userId) {
  const total = db.prepare('SELECT COUNT(*) c FROM lectures WHERE user_id = ?').get(userId).c;
  const ready = db.prepare('SELECT COUNT(*) c FROM lectures WHERE user_id = ? AND status = ?').get(userId, 'ready').c;
  if (!total) return 0;
